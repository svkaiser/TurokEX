...
void Z_Init(void)
{
    ...
    Cmd_AddCommand("z_stats", FCmd_ZoneStats);
...
void (Z_ChangeTag)(void *ptr, int tag, const char *file, int line)
{
    ...
    if(block->id != ZONEID)
        Com_Error("Z_ChangeTag: block without a ZONEID! (%s:%d)",
        file, line);
    
    if(tag >= PU_PURGELEVEL && block->user == NULL)
        Com_Error("Z_ChangeTag: an owner is required for purgable blocks (%s:%d)", file, line);
...
int Z_TagUsage(int tag)
{
    ...
    if(tag < 0 || tag >= PU_MAX)
        Com_Error("Z_TagUsage: tag out of range: %i", tag);
