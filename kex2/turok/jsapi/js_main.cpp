...
JSObject *J_AddObject(JSClass *cls,
                      JSFunctionSpec *func,
                      JSPropertySpec *prop,
                      ...
                      const char *name, JSContext *cx, JSObject *obj)
{
    ...
    if(!(newobj = JS_DefineObject(cx, obj, name, cls, NULL, 0)))
        common.Error("J_AddObject: Failed to create a new class for %s", name);

    if(prop)
    {
        if(!JS_DefineProperties(cx, newobj, prop))
             common.Error("J_AddObject: Failed to define properties for class %s", name);
    }

    if(func)
    {
        if(!JS_DefineFunctions(cx, newobj, func))
            common.Error("J_AddObject: Failed to define functions for class %s", name);
    }

    if(constant)
    {
        if(!JS_DefineConstDoubles(cx, newobj, constant))
            common.Error("J_AddObject: Failed to define constants for class %s", name);
    }
                        ...
                        char *file, int line)
{
    ...
    if(!JS_ReportWarning(cx, "New Object(%s) called at:\n%s : %i\n", name, file, line))
        common.Printf("New Object(%s): %s : %i\n", name, file, line);
...
JSBool J_NewDoubleValueLog(JSContext *cx, jsdouble d, jsval *rval, char *file, int line)
{
    if(!JS_ReportWarning(cx, "New Double(%f) called at:\n%s : %i\n", d, file, line))
        common.Printf("New Double(%f): %s : %i\n", d, file, line);
...
js_scrobj_t *J_LoadScriptObject(const char *name, char *buffer, int size)
{
    ...
    {
        common.Error("J_LoadScriptObject: \"%s\" is too long", name);
    }
...
void J_ExecScriptObj(js_scrobj_t *scobj)
{
    ...
    {
        JS_ReportPendingException(cx);
        common.Warning("Unable to execute %s\n", scobj->name);
    }
}
...
static void FCmd_LogJS(void)
{
    ...
    {
        common.Printf("Usage: jslog <1=enable, 0=disable>\n");
        return;
    }

    enable = atoi(command.GetArgv(1));
...
static void FCmd_JS(void)
{
    ...
    if(command.GetArgc() < 2)
    {
        common.Printf("Usage: js <code>\n");
        return;
    }

    buffer = command.GetArgv(1);
...
static void FCmd_JSFile(void)
{
    ...
    if(command.GetArgc() < 2)
    {
        common.Printf("Usage: jsfile <filename>\n");
        return;
    }
    ...
    if(script = JS_CompileFile(cx, obj, kva("%s\\%s",
        cvarBasePath.GetValue(), command.GetArgv(1))))
    {
        JS_ExecuteScript(cx, obj, script, &result);
        JS_ReportPendingException(cx);
        JS_DestroyScript(cx, script);
...
static void FCmd_JSLoad(void)
{
    if(command.GetArgc() < 2)
    {
        common.Printf("Usage: jsload <filename>\n");
        return;
    }

    if(J_LoadScript(command.GetArgv(1)))
    {
        common.Printf("Script loaded\n");
    }
}
...
static void FCmd_JSExec(void)
{
    ...
    if(command.GetArgc() < 2)
    {
        common.Printf("Usage: jsexec <name>\n");
        return;
    }

    if(!(scrobj = J_FindScript(command.GetArgv(1))))
    {
        common.Printf("\"%s\" is not loaded\n", command.GetArgv(1));
        return;
    }
