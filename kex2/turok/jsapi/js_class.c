...
void JClass_InitObject(void)
{
    ...
    if(!JS_GetProperty(js_context, js_gobject, "class", &val))
        Com_Error("JS_InitClassObject: javascript object 'class' not found");
    if(!JS_ValueToObject(js_context, val, &classObj.object))
        Com_Error("JS_InitClassObject: unable to get class object");
    if(!JS_GetProperty(js_context, classObj.object, "find", &val))
        Com_Error("JS_InitClassObject: unable to get 'find' property");
    if(!JS_ValueToObject(js_context, val, &classObj.findObj))
        Com_Error("JS_InitClassObject: failed to setup 'find' object");
    if(!JS_ObjectIsFunction(js_context, classObj.findObj))
        Com_Error("JS_InitClassObject: 'find' object is not a function");
    if(!JS_GetProperty(js_context, classObj.object, "create", &val))
        Com_Error("JS_InitClassObject: unable to get 'create' property");
    if(!JS_ValueToObject(js_context, val, &classObj.createObj))
        Com_Error("JS_InitClassObject: failed to setup 'create' object");
    if(!JS_ObjectIsFunction(js_context, classObj.createObj))
        Com_Error("JS_InitClassObject: 'create' object is not a function");
