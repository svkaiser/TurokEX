...
JS_PROP_FUNC_GET(NServer)
{
    ...
    case NS_TOTALCLIENTS:
        ...
        JS_SET_RVAL(cx, vp, INT_TO_JSVAL(server.GetMaxClients()));
        return JS_TRUE;

    case NS_TIME:
        //return JS_NewNumberValue(cx, server.time, vp);
        JS_SET_RVAL(cx, vp, INT_TO_JSVAL(server.GetTime()));
        return JS_TRUE;

    case NS_RUNTIME:
        //return JS_NewNumberValue(cx, server.runtime, vp);
        JS_SET_RVAL(cx, vp, INT_TO_JSVAL(server.GetRunTime()));
        return JS_TRUE;

    case NS_TICS:
        //return JS_NewNumberValue(cx, server.tics, vp);
        JS_SET_RVAL(cx, vp, INT_TO_JSVAL(server.GetTicks()));
        return JS_TRUE;

    case NS_HOST:
        JS_NEWOBJECTPOOL(server.GetHost(), Host);
        //JS_NEWOBJECT_SETPRIVATE(server.host, NULL);
        return JS_TRUE;

    case NS_NETEVENT:
        JS_NEWOBJECTPOOL(server.GetEvent(), NetEvent);
...
JS_PROP_FUNC_SET(NServer)
{
    ...
    case NS_TIME:
        ...
        server.SetTime((int)val);
        return JS_TRUE;

    case NS_RUNTIME:
        JS_GETNUMBER(val, vp, 0);
        server.SetRunTime((float)val);
        return JS_TRUE;

    case NS_TICS:
        JS_GETNUMBER(val, vp, 0);
        server.SetTicks((int)val);
        return JS_TRUE;
...
JS_FASTNATIVE_BEGIN(NServer, getPeerAddress)
{
    ...
    JS_RETURNSTRING(vp, server.GetPeerAddress(ev));
    return JS_TRUE;
}
...
JS_FASTNATIVE_BEGIN(NServer, getClientID)
{
    ...
    JS_SET_RVAL(cx, vp, INT_TO_JSVAL(server.GetClientID(peer)));
    return JS_TRUE;
}
