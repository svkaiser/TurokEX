...
JS_PROP_FUNC_GET(NClient)
{
    ...
    {
    case CL_STATE:
        JS_SET_RVAL(cx, vp, INT_TO_JSVAL(client.GetState()));
        return JS_TRUE;
    ...
    case CL_HOST:
        JS_NEWOBJECTPOOL(client.GetHost(), Host);
        return JS_TRUE;

    case CL_PEER:
        JS_NEWOBJECTPOOL(client.GetPeer(), Peer);
        return JS_TRUE;

    case CL_NETEVENT:
        JS_NEWOBJECTPOOL(client.GetEvent(), NetEvent);
        return JS_TRUE;

    case CL_ID:
        JS_SET_RVAL(cx, vp, INT_TO_JSVAL(client.id));
        return JS_TRUE;
