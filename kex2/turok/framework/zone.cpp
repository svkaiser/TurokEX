...
void Z_Init(void)
{
    ...
    command.Add("z_stats", FCmd_ZoneStats);
...
void (Z_Free)(void* ptr, const char *file, int line)
{
    ...
    if(block->id != ZONEID)
        common.Error("Z_Free: freed a pointer without ZONEID (%s:%d)", file, line);
...
static kbool Z_ClearCache(int size)
{
    memblock_t *block;
...
void *(Z_Malloc)(int size, int tag, void **user, const char *file, int line)
{
    ...
    if(tag < 0 || tag >= PU_MAX)
        common.Error("Z_Malloc: tag out of range: %i (%s:%d)", tag, file, line);
    ...
    if(!newblock)
        common.Error("Z_Malloc: failed on allocation of %u bytes (%s:%d)", size, file, line);
...
void *(Z_Realloc)(void *ptr, int size, int tag, void **user, const char *file, int line)
{
    ...
    if(tag < 0 || tag >= PU_MAX)
        common.Error("Z_Realloc: tag out of range: %i (%s:%d)", tag, file, line);
    
    if(user == NULL && tag >= PU_PURGELEVEL)
        common.Error("Z_Realloc: an owner is required for purgable blocks (%s:%d)", file, line);
    ...
    if(block->id != ZONEID)
        common.Error("Z_Realloc: Reallocated a pointer without ZONEID (%s:%d)", file, line);
    ...
    if(!newblock)
        common.Error("Z_Realloc: failed on allocation of %u bytes (%s:%d)", size, file, line);
...
void (Z_FreeTags)(int lowtag, int hightag, const char *file, int line)
{
    ...
    for(i = lowtag; i <= hightag; ++i)
    {
        ...
        for(block = allocated_blocks[i]; block != NULL;)
        {
            ...
            if(block->id != ZONEID)
                common.Error("Z_FreeTags: Changed a tag without ZONEID (%s:%d)", file, line);
...
char *(Z_Strndup)(const char *s, unsigned int n, int tag, void **user, const char *file, int line)
{
	...
	return strcpy((char*)((Z_Malloc)(len+1, tag, user, file, line)), s);
}
...
void (Z_CheckHeap)(const char *file, int line)
{
    ...
    for(i = 0; i < PU_MAX; ++i)
    {
        ...
        {
            if(block->id != ZONEID)
                common.Error("Z_CheckHeap: Block without a ZONEID! (%s:%d)", file, line);
            
            if(block->prev != prev)
                common.Error("Z_CheckHeap: Doubly-linked list corrupted! (%s:%d)", file, line);
...
int (Z_CheckTag)(void *ptr, const char *file, int line)
{
    ...
    if(block->id != ZONEID)
        common.Error("Z_CheckTag: block doesn't have ZONEID (%s:%d)", file, line);
...
void (Z_ChangeTag)(void *ptr, int tag, const char *file, int line)
{
    ...
    if(block->id != ZONEID)
        common.Error("Z_ChangeTag: block without a ZONEID! (%s:%d)",
        file, line);
    
    if(tag >= PU_PURGELEVEL && block->user == NULL)
        common.Error("Z_ChangeTag: an owner is required for purgable blocks (%s:%d)", file, line);
...
void Z_CleanupTag(int tag)
{
    ...
    if(tag < 0 || tag >= PU_MAX)
        common.Error("Z_CleanupTag: tag out of range: %i", tag);
...
int Z_TagUsage(int tag)
{
    ...
    if(tag < 0 || tag >= PU_MAX)
        common.Error("Z_TagUsage: tag out of range: %i", tag);
